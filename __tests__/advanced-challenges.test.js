const { getEvenNumbers, flipBooleans, translateKey, findFirstDentist, tallyPeopleInManchester, getPugOwners, pluraliseKeys, getWordLengths, getPalindromes, replaceLettersWithXs, validMobileNumber, sumDigitsFromString, getWilliams, getFactorials, largestNumber, generateMatrix, findWrongWayFruit, pairDNA, tallyHashtagsAndMentions } = require("../advanced-challenges/advanced-challenges");
...
describe("tallyHashtagsAndMentions", () => {
  ...
  it("returns {hashtags: 0, mentions: 0} if it finds none", () => {
    expect(tallyHashtagsAndMentions("hello this is a tweet guaranteed to get very little engagement")).toEqual({ hashtags: 0, mentions: 0 });
  });
  ...
  it("finds multiple hashtags and mentions and returns that number", () => {
    ...
    expect(tallyHashtagsAndMentions('"So excited to start at @northcoders on Monday! #learntocode #codingbootcamp"')).toEqual({ hashtags: 2, mentions: 1 });
  });
});
